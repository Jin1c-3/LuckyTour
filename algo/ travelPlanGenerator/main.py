...
def getGptResponse(data):
    ...
    """
    # 提示词模板
    template = """"作为旅行规划助手，''你的任务是接收关于旅客类型、预算水平、旅行日期和兴趣的信息，并按json格式回答。并按时间顺序规划景点(" 
    ...
    selectedInterests = ', '.join(data.get('tag', []))  # 假设这是一个列表
    ...
    memory = ConversationBufferWindowMemory(llm=llm, k=2, memory_key="chat_history")
    # 设置GPT生成的prompt模板
