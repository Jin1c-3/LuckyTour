...
class AuthCodeRegister extends StatefulWidget {
  const AuthCodeRegister({super.key});

  @override
  State<AuthCodeRegister> createState() => _AuthCodeRegisterState();
}

class _AuthCodeRegisterState extends State<AuthCodeRegister> {
  ...
  void _verify(String pin) async {
    if (formKey.currentState!.validate()) {
      final result = await UserApi().createUser({
      ...
      if (result['code'] == 200) {
        ...
        Get.until((route) => Get.currentRoute == "/login_or_register");
      } else {
        Get.snackbar(
          '失败',
          result['message'],
