...
class ChatGuide extends StatefulWidget {
  const ChatGuide({super.key});

  @override
  State<ChatGuide> createState() => _ChatGuideState();
}

class _ChatGuideState extends State<ChatGuide> {
  ...
  void _getInfo() async {
    final result = await PlanApi().getAiGuide({
      "thread_id": threadId,
      "location": planController.guideLocation.value,
      "ask": "",
    });
    setState(() {
      isLoading = false;
  ...
  void initState() {
    ...
    _addMessage(
      types.TextMessage(
        ...
        createdAt: DateTime.now().millisecondsSinceEpoch,
        id: randomString(),
        text: "📍${planController.guideLocation}",
      ),
    );
    setState(() {
      isLoading = true;
