...
class PlanChatRebuild extends StatefulWidget {
  const PlanChatRebuild({super.key});

  @override
  State<PlanChatRebuild> createState() => _PlanChatRebuildState();
}

class _PlanChatRebuildState extends State<PlanChatRebuild> {
  ...
  void sendInit() async {
    final result = await StrategyApi().createPlanByTemplate({
      "uid": planController.selectedStrategyItem['uid'],
      "pid": planController.selectedStrategyItem['pid'],
    });
    setState(() {
      isLoading = false;
    });
  ...
  void initState() {
    ...
    _addMessage(
      types.TextMessage(
        ...
        id: randomString(),
        text: "你好，我是您的旅行助手小云，请告诉我你对当前生成计划的修改意见，我将给予你选择的模板进行调整",
      ),
    );
    sendInit();
  }
